...
class LossWrapper(torch.nn.Module):
    ...
    def forward(self, fc_feats, att_feats, labels, masks, att_masks, seqtree, seqtree_idx, seqtree_mask,
                gts, gt_indices, sc_flag, struc_flag):
        opt = self.opt
        
        out = {}
        if struc_flag:
            raise NotImplementedError("struc_flag for tree generating not implemented.")
            ...
            loss = self.crit(self.model(fc_feats, att_feats, labels, att_masks, seqtree, seqtree_idx, seqtree_mask), seqtree, seqtree_mask)
        else:
            raise NotImplementedError("sc_flag for tree generating not implemented.")
            # self.model.eval()
            # with torch.no_grad():
